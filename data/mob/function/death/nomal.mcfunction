...
execute if entity @s[tag=ReceiveMelee] as @a if score @s player_id = $who Temporary run \
    scoreboard players operation @s sword_level_all_xp += @n[tag=death_xp_calc] mob_xp

execute if entity @s[tag=ReceiveMagic] as @a if score @s player_id = $who Temporary run \
    scoreboard players operation @s magic_level_all_xp += @n[tag=death_xp_calc] mob_xp

execute as @a if score @s player_id = $who Temporary run \

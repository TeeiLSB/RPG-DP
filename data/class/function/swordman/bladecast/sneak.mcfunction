...
execute unless predicate player:is_sneaking_false if score @s sneak_cd matches 0 run function class:swordman/bladecast/sword_slot
execute unless predicate player:is_sneaking_false if score @s sneak_cd matches 0 run scoreboard players set @s sneak_cd 4

execute if score @s sneak_cd matches 4 run playsound ui.button.click master @s ~ ~ ~ 1 2
